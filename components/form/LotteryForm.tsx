...
const LotteryForm = () => {
    ...
    return (
        <form className='space-y-3' onSubmit={handleSubmit}>
            ...
            <Button variant={"ghost"} size={"lg"} disabled={new Date(lotterEndTime).getTime()< Date.now()} className=' bg-gradient-to-r transition-all hover:from-violet-500 hover:to-pink-500 duration-700 from-pink-500 to-violet-500 w-full rounded-full text-white hover:text-white'>{isLoading ? "Entering the lottery" : "Enter"}</Button>
        </form>
    )
}
